...
#   - one_artifial_target = True -> one wants one radial and one tangential target computation and visualisation that are near each 
...
if head_model=="Sphere":
    ...
    target_element_indices_radial = np.array([
        ast.literal_eval(configs.get('data_head', 'single_radial_indice'))
    ], dtype=int)
    radial_vectors = np.array([
        ast.literal_eval(configs.get('data_head', 'single_radial_moment'))
    ], dtype=float)
    target_element_indices_tangential = np.array([
        ast.literal_eval(configs.get('data_head', 'single_tangential_indice'))
    ], dtype=int)
...
if grey_matter_non_target:
    gr_or = "GR"
    omega[grey_matter_indices*3, 0] = omega_value_non_target  
...
rad_stats = []
tan_stats = []
print(target_element_indices_radial.shape)
...
for i in range(target_element_indices_radial.shape[0]):
    ...
    exit_status, I_radial, optimal_lagrange_eq, optimal_lagrange_ineq, opt_func_value, iteration_count = active_set_evaluation(alpha,beta,B_sliced,e_tilde_sliced, n_chan,omega_sliced,eps,I_total=I_total,I_ind= I_ind,verbosity=verbosity)
    ...
    if check_whole_domain_constraints(B_sliced,I_radial,omega_sliced,eps,tolerance):
...
for i in range(target_element_indices_tangential.shape[0]):
    ...
    print(f'Starting Active Set algorithm for target......')
    t_start_iteration = time.time()
    exit_status, I_tangential, optimal_lagrange_eq, optimal_lagrange_ineq, opt_func_value, iteration_count = active_set_evaluation(alpha,beta,B_sliced,e_tilde_sliced, n_chan,omega_sliced,eps,I_total=I_total,I_ind= I_ind,verbosity=verbosity)
    t_end_iteration = time.time()
    print(f"[TIME] Active Set solving for target: {t_end_iteration - t_start_iteration:.3f}s")
...
# Write Vtk files and compute stats for visualization for one target 

...
# Essentially the same as in active_set_test.py but with for loop over epsilons and comparing stats
...
alpha = 10
beta = 10
epsilons = [0.1,4,2000] # varying epsilons for the thesis
omega_value_non_target = 1.0 # grey matter
omega_value = 0.000001  # target region &  non grey matter
...
# Iterate over the epsilons and compute the solutions
for i in range(len(epsilons)):
    ...
    for i in range(len(target_element_indices_radial)):
        ...
        exit_status, I_radial, optimal_lagrange_eq, optimal_lagrange_ineq, opt_func_value, iteration_count = active_set_evaluation(alpha,beta,B_sliced,e_tilde_sliced, n_chan,omega_sliced,eps,verbosity=verbosity)
    ...
    for i in range(len(target_element_indices_tangential)):
        ...
        print(f'Starting Active Set algorithm for target......')
        t_start_iteration = time.time()
        exit_status, I_tangential, optimal_lagrange_eq, optimal_lagrange_ineq, opt_func_value, iteration_count = active_set_evaluation(alpha,beta,B_sliced,e_tilde_sliced, n_chan,omega_sliced,eps,verbosity=verbosity)
        t_end_iteration = time.time()
        print(f"[TIME] Active Set solving for target: {t_end_iteration - t_start_iteration:.3f}s")
    ...
    include_ref = True  # set to some value to include reference penalty in run directory name
